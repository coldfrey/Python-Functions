...
def generateTaxonObjects(inputCSV):
  ...
  for product in unique_products:
      ...
      if pd.isna(product_df.loc[product_df['Product Type'] == product, 'Barcode EAN'].iloc[0]):
            product_df.loc[product_df['Product Type'] == product, 'Barcode EAN'] = str(int(variant_df.loc[variant_df['Product Name'].str.contains(product), 'Barcode EAN'].iloc[0])) + '-P'

  # If OQI is NaN, assign 1
  product_df['OQI'] = product_df['OQI'].fillna(1)
  print(product_df['OQI'])
